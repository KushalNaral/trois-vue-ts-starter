<template>
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
            ...
            <div class="container mx-auto px-4 py-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-xl font-bold text-slate-800">Room Designer Studio</h1>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="px-3 py-1 bg-slate-100 rounded-full text-sm font-medium text-slate-700">
        ...
        <div class="flex h-[calc(100vh-80px)]">
            <!-- Left Sidebar - Item Library (unchanged) -->
            <div class="w-80 bg-white/70 backdrop-blur-xl border-r border-slate-200 overflow-hidden flex flex-col">
                <div class="p-4 border-b border-slate-200">
                    <h2 class="text-lg font-semibold text-slate-800 mb-3">Item Library</h2>
                    <div class="flex gap-1 bg-slate-100 rounded-lg p-1">
                        <button v-for="category in categories" :key="category" @click="selectedCategory = category"
                            :class="[
                                'px-3 py-1 text-sm font-medium rounded-md transition-all capitalize',
                                selectedCategory === category ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-600 hover:text-slate-800',
                            ]">
                            {{ category }}
                        </button>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-4 space-y-3">
                    <div v-for="(item, key) in filteredItems" :key="key" @click="selectItem(key)" :class="[
                        ...
                        selectedItem === key ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-slate-200 bg-white/50 hover:bg-slate-50',
                    ]">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                            ...
                            <button @click.stop="toggleItemVisibility(key)"
                                :class="['p-2 rounded-lg transition-colors', item.visible ? 'text-blue-600 hover:bg-blue-100' : 'text-slate-400 hover:bg-slate-100']">
                                {{ item.visible ? '👁️' : '👁️‍🗨️' }}
                            </button>
                        </div>
                    </div>
            ...
            <div class="flex-1 relative">
                <Renderer ref="rendererC" class="w-full h-full" antialias :alpha="true" :orbit-ctrl="{
                    enableDamping: true,
                    enablePan: false,
                    minDistance: 8,
                    maxDistance: 15,
                    ...
                    <Scene>
                        ...
                        <DirectionalLight :position="{ x: 10, y: 10, z: 5 }" :intensity="0.8" :cast-shadow="true" />
                        <PointLight :position="{ x: -5, y: 8, z: 2 }" :intensity="0.4" color="#ffd700" />
                        <PointLight :position="{ x: 5, y: 3, z: 5 }" :intensity="0.3" color="#87ceeb" />

                        <!-- Room Structure with Enhanced Materials -->
                        <Plane :width="20" :height="20" :rotation="{ x: -Math.PI / 2 }" :position="{ y: -2 }"
                            :receive-shadow="true">
                            <LambertMaterial color="#f8fafc" />
                        </Plane>
                        <Plane :width="20" :height="12" :position="{ y: 4, z: -8 }" :receive-shadow="true">
                            <LambertMaterial color="#f1f5f9" />
                        </Plane>
                        <Plane :width="16" :height="12" :rotation="{ y: Math.PI / 2 }"
                            :position="{ x: -10, y: 4, z: 0 }" :receive-shadow="true">
                            <LambertMaterial color="#f8fafc" />
                        </Plane>
                        <Plane :width="16" :height="12" :rotation="{ y: -Math.PI / 2 }"
                            :position="{ x: 10, y: 4, z: 0 }" :receive-shadow="true">
                        ...
                        <Box v-if="roomItems.sofa.visible" ref="sofaRef" :width="4" :height="1.5" :depth="2"
                            :position="roomItems.sofa.position" :rotation="roomItems.sofa.rotation"
                            :scale="roomItems.sofa.scaleVector" :receive-shadow="true" :cast-shadow="true"
                            @click="selectItem('sofa')" @pointerover="highlightItem('sofa')"
                            @pointerout="unhighlightItem('sofa')">
                            <LambertMaterial :color="roomItems.sofa.color" />
                        </Box>

                        <Box v-if="roomItems.coffeeTable.visible" ref="coffeeTableRef" :width="2" :height="0.3"
                            :depth="1" :position="roomItems.coffeeTable.position"
                            :rotation="roomItems.coffeeTable.rotation" :scale="roomItems.coffeeTable.scaleVector"
                            :receive-shadow="true" :cast-shadow="true" @click="selectItem('coffeeTable')"
                            @pointerover="highlightItem('coffeeTable')" @pointerout="unhighlightItem('coffeeTable')">
                            <LambertMaterial :color="roomItems.coffeeTable.color" />
                        </Box>

                        <Box v-if="roomItems.tvStand.visible" ref="tvStandRef" :width="3" :height="0.8" :depth="0.8"
                            :position="roomItems.tvStand.position" :rotation="roomItems.tvStand.rotation"
                            :scale="roomItems.tvStand.scaleVector" :receive-shadow="true" :cast-shadow="true"
                            @click="selectItem('tvStand')" @pointerover="highlightItem('tvStand')"
                            @pointerout="unhighlightItem('tvStand')">
                            <LambertMaterial :color="roomItems.tvStand.color" />
                        </Box>

                        <Box v-if="roomItems.tv.visible" ref="tvRef" :width="2.5" :height="1.5" :depth="0.1"
                            :position="roomItems.tv.position" :rotation="roomItems.tv.rotation"
                            :scale="roomItems.tv.scaleVector" :receive-shadow="true" :cast-shadow="true"
                            @click="selectItem('tv')" @pointerover="highlightItem('tv')"
                            @pointerout="unhighlightItem('tv')">
                            <LambertMaterial :color="roomItems.tv.color" />
                        </Box>

                        <Cylinder v-if="roomItems.lamp.visible" ref="lampRef" :radius="0.15" :height="2"
                            :position="roomItems.lamp.position" :rotation="roomItems.lamp.rotation"
                            :scale="roomItems.lamp.scaleVector" :receive-shadow="true" :cast-shadow="true"
                            @click="selectItem('lamp')" @pointerover="highlightItem('lamp')"
                            @pointerout="unhighlightItem('lamp')">
                            <LambertMaterial :color="roomItems.lamp.color" />
                        </Cylinder>

                        <Box v-if="roomItems.bookshelf.visible" ref="bookshelfRef" :width="1.5" :height="4" :depth="0.8"
                            :position="roomItems.bookshelf.position" :rotation="roomItems.bookshelf.rotation"
                            :scale="roomItems.bookshelf.scaleVector" :receive-shadow="true" :cast-shadow="true"
                            @click="selectItem('bookshelf')" @pointerover="highlightItem('bookshelf')"
                            @pointerout="unhighlightItem('bookshelf')">
                            <LambertMaterial :color="roomItems.bookshelf.color" />
                        </Box>

                        <Sphere v-if="roomItems.plant.visible" ref="plantRef" :radius="0.5"
                            :position="roomItems.plant.position" :rotation="roomItems.plant.rotation"
                            :scale="roomItems.plant.scaleVector" :receive-shadow="true" :cast-shadow="true"
                            @click="selectItem('plant')" @pointerover="highlightItem('plant')"
                ...
                <div class="absolute top-4 right-4 bg-white/80 backdrop-blur-md rounded-lg p-2 shadow-lg">
                    <div class="flex gap-1">
                        ...
                        <button @click="toggleDragControls" :class="[
                            ...
                            dragEnabled ? 'text-blue-600 bg-blue-100' : 'text-slate-600 hover:bg-slate-100'
                        ]" title="Toggle Drag Controls">
                            ✋
                        </button>
                        <button class="p-2 text-slate-600 hover:bg-slate-100 rounded-md transition-colors"
                            title="Settings">
                            ⚙️
                        </button>
                    </div>
                ...
                <div class="absolute top-4 left-4 bg-black/70 backdrop-blur-md rounded-lg p-3 text-white text-sm">
                    ...
                    <div>
                        Template: <span class="text-blue-400">{{ templates[selectedTemplate]?.name }}</span>
                    </div>
                </div>
            </div>
            ...
            <div
                :class="['w-96 bg-white/70 backdrop-blur-xl border-l border-slate-200 transition-transform duration-300', selectedItem ? 'translate-x-0' : 'translate-x-full']">
                <div v-if="selectedItem" class="h-full flex flex-col">
                    <div class="p-6 border-b border-slate-200">
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="text-xl font-semibold text-slate-800">Customize Item</h2>
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 class="font-semibold text-slate-800">{{ selectedItemData?.name }}</h3>
                                <p class="text-sm text-slate-500">{{ selectedItemData?.description }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="flex-1 overflow-y-auto p-6 space-y-6">
                        <!-- Style Variants -->
                        <div>
                            <h4 class="font-medium text-slate-800 mb-3 flex items-center gap-2">🎨 Style Variants</h4>
                            <div class="grid grid-cols-2 gap-3">
                                <div v-for="(variant, index) in getItemVariants(selectedItem)" :key="index"
                                    @click="applyVariant(selectedItem, variant)" :class="[
                                        ...
                                        isCurrentVariant(selectedItem, variant) ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white hover:border-slate-300',
                                    ]">
                                    <div class="w-12 h-12 mx-auto mb-2 rounded-lg border-2 border-slate-200 shadow-inner"
                                        :style="{ backgroundColor: variant.color }"></div>
                                    <p class="text-sm font-medium text-slate-700">{{ variant.name }}</p>
                        ...
                        <!-- Transform Controls -->
                        <div>
                            <h4 class="font-medium text-slate-800 mb-3 flex items-center gap-2">📐 Transform</h4>
                            <div class="space-y-4">
                                <div>
                                    <label class="text-sm text-slate-600 mb-2 block flex items-center justify-between">
                                        Scale
                                    ...
                                    <input type="range" :value="selectedItemData?.scale * 100"
                                        @input="updateItemScale($event.target.value / 100)" min="50" max="200" step="10"
                                ...
                                <div>
                                    <label class="text-sm text-slate-600 mb-2 block flex items-center justify-between">
                                        Rotation Y
                                        <span class="font-mono text-xs bg-slate-100 px-2 py-1 rounded">{{
                                            Math.round((selectedItemData?.rotation.y * 180) / Math.PI) }}°</span>
                                    </label>
                                    <input type="range" :value="(selectedItemData?.rotation.y * 180) / Math.PI"
                                        @input="updateItemRotation('y', ($event.target.value * Math.PI) / 180)" min="0"
                                        max="360" step="15"
                                        class="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer slider" />
                                </div>
                            </div>
                ...
                <div v-else class="h-full flex items-center justify-center p-6">
                    <div class="text-center">
                        ...
                        <p class="text-sm text-slate-500 max-w-sm">
                            Select an item from the library to customize its appearance and properties.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50">
            <div class="bg-white/90 backdrop-blur-xl border border-slate-200 rounded-full px-6 py-3 shadow-lg">
                <div class="flex items-center gap-4">
...
const itemVariants = {
    sofa: [
        { name: 'Classic Leather', color: '#8B4513', style: 'Traditional', mood: 'Warm' },
        { name: 'Modern Gray', color: '#708090', style: 'Contemporary', mood: 'Cool' },
        { name: 'Scandinavian Beige', color: '#F5DEB3', style: 'Nordic', mood: 'Cozy' },
        { name: 'Statement Navy', color: '#1e3a8a', style: 'Bold', mood: 'Sophisticated' },
        { name: 'Velvet Emerald', color: '#059669', style: 'Luxury', mood: 'Rich' },
        { name: 'Minimalist White', color: '#f8fafc', style: 'Clean', mood: 'Fresh' },
    ],
    coffeeTable: [
        { name: 'Dark Walnut', color: '#654321', style: 'Traditional', mood: 'Warm' },
        { name: 'Glass Modern', color: '#e0f2fe', style: 'Contemporary', mood: 'Light' },
        { name: 'Industrial Metal', color: '#475569', style: 'Urban', mood: 'Edgy' },
        { name: 'Marble Luxury', color: '#f1f5f9', style: 'Premium', mood: 'Elegant' },
    ],
    tv: [
        { name: 'Classic Black', color: '#000000', style: 'Standard', mood: 'Neutral' },
        { name: 'Premium Silver', color: '#cbd5e1', style: 'Modern', mood: 'Sleek' },
        { name: 'Curved OLED', color: '#1e293b', style: 'Futuristic', mood: 'Advanced' },
        { name: 'Art Frame', color: '#92400e', style: 'Decorative', mood: 'Artistic' },
    ],
    lamp: [
        { name: 'Warm Amber', color: '#f59e0b', style: 'Cozy', mood: 'Relaxing' },
        { name: 'Cool White', color: '#f8fafc', style: 'Modern', mood: 'Crisp' },
        { name: 'Vintage Brass', color: '#d97706', style: 'Retro', mood: 'Nostalgic' },
        { name: 'RGB Smart', color: '#ec4899', style: 'Tech', mood: 'Dynamic' },
    ],
    plant: [
        { name: 'Fiddle Leaf', color: '#16a34a', style: 'Tropical', mood: 'Fresh' },
        { name: 'Orchid Bloom', color: '#c084fc', style: 'Elegant', mood: 'Delicate' },
        { name: 'Desert Cactus', color: '#65a30d', style: 'Minimal', mood: 'Zen' },
        { name: 'Bamboo Grove', color: '#84cc16', style: 'Asian', mood: 'Peaceful' },
    ],
    tvStand: [
        { name: 'Dark Slate', color: '#2F4F4F', style: 'Modern', mood: 'Sleek' },
        { name: 'Wooden Oak', color: '#8B4513', style: 'Traditional', mood: 'Warm' },
        { name: 'White Gloss', color: '#f8fafc', style: 'Contemporary', mood: 'Clean' },
        { name: 'Industrial Steel', color: '#475569', style: 'Urban', mood: 'Edgy' },
    ],
    bookshelf: [
        { name: 'Rustic Wood', color: '#8B4513', style: 'Traditional', mood: 'Warm' },
        { name: 'White Modern', color: '#f8fafc', style: 'Contemporary', mood: 'Clean' },
        { name: 'Black Metal', color: '#1e293b', style: 'Industrial', mood: 'Bold' },
        { name: 'Walnut Premium', color: '#654321', style: 'Luxury', mood: 'Elegant' },
    ],
};
...
const filteredItems = computed(() => {
    ...
    return Object.fromEntries(
        Object.entries(roomItems).filter(([_, item]) => item.category === selectedCategory.value)
    );
});
...
const selectedItemData = computed(() => {
    return selectedItem.value ? roomItems[selectedItem.value as keyof typeof roomItems] : null;
});
...
onMounted(() => {
    ...
    if (renderer && scene && camera) {
        ...
        const draggableObjects = Object.keys(roomItems)
            ...
            .filter((obj) => obj);
        dragControls.value = new DragControls(draggableObjects, camera, renderer.domElement);
        dragControls.value.enabled = dragEnabled.value;

        // Update position on drag
        dragControls.value.addEventListener('drag', (event) => {
            const object = event.object;
            const key = Object.keys(roomItems).find((k) => refs[`${k}Ref`]?.three === object);
            if (key) {
                const item = roomItems[key as keyof typeof roomItems];
                item.position = {
                    x: object.position.x,
        ...
        const onClick = (event: MouseEvent) => {
            ...
            if (intersects.length > 0) {
                const key = Object.keys(roomItems).find((k) => refs[`${k}Ref`]?.three === intersects[0].object);
                if (key) {
                    selectItem(key);
                }
            }
        ...
        // FPS counter
        const updateFPS = () => {
            fps.value = Math.round(rendererC.value?.renderer.info.render.frameRate || 60);
